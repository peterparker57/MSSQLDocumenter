...
class DatabaseDocumenter:
    ...
    async def search_documentation(self, query: str, n_results: int = 5) -> List[Dict[str, Any]]:
        """Search documentation using natural language."""
        try:
            # First, analyze the search intent using LLM
            intent = await self.llm_client.analyze_search_intent(query)
            logger.info(f"Search intent analysis: {intent}")
            
            # Use the processed search query from the intent analysis
            search_query = intent.get("search_query", query)
            
            # Perform the vector search
            results = self.vector_store.search(search_query, n_results)
            
            # Format results based on intent
            formatted_results = []
            for result in results:
                # Filter by object type if specified
                if intent.get("object_type") != "any" and result["metadata"]["type"] != intent.get("object_type"):
                    continue
                    
                # Format content based on detail level
                detail_level = intent.get("detail_level", "full_details")
                if detail_level == "names_only":
                    formatted_content = f"{result['metadata']['schema']}.{result['metadata']['name']}"
                elif detail_level == "basic_info":
                    formatted_content = f"{result['metadata']['schema']}.{result['metadata']['name']} ({result['metadata']['type']})"
                else:
                    # full_details - use the full content
